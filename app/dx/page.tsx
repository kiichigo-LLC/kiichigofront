import { DxContent } from "@/components/dx/DxContent";
...
const PATH = "/dx";
const FALLBACK_TITLE = "DX推進方針について";

export async function generateMetadata(): Promise<Metadata> {
  const pageTitle = await resolvePageTitle("dx", FALLBACK_TITLE);
...
export default async function DxPage() {
  const [navNew, pageTitle] = await Promise.all([
    getNavNewFlags(),
    resolvePageTitle("dx", FALLBACK_TITLE),
  ]);
